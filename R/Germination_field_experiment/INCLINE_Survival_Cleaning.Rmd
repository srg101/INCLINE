...
# Rename levels of variables and create ID columns
  ...
  mutate(plot_seedling_ID = paste0(blockID, "_", experiment, "_",
                                   OTC, "_", vegetation)) %>%
...
# Plot: Number = background check for germination, non-number ? C, W = actual germination trial. Trial/Background.
# Add to uniqueID as well. 

# ISSUES: Still a work in process....
# We have cases where there are multiple measures on 'present' for an individual for a given campaign.
...
# "Ver_alp_Gud_2_E_W_NoVeg_1", "Ver_alp_Gud_2_E_W_NoVeg_2" and "Ver_alp_Gud_2_E_W_NoVeg_3": First campaign 2x dates 2020-06-30 (no) and 2020-07-17 (yes), but dates are spread apart. Same registrator (RG). All pres in all bar 1x1 campaign option.
# "Ver_alp_Gud_4_E_W_Veg_5": Has 4 records for campaign 7, all same date 3x no, 1x yes, same registrator (CZ). Pres only in 2,3 and 1x7.
# "Ver_alp_Gud_5_E_W_NoVeg_13": Has 2x campaign 2, one yes one no, same date and registrar (RG). Pres only in 1x2
# "Ver_alp_Gud_5_E_W_NoVeg_16": Has 2x campaign 3, one yes one no, same date and registrar(RG)- Pres only in 2, 1x3, and 4.
# "Ver_alp_Gud_5_E_W_Veg_2": Has 2x campaign 2, one yes one no, same date and registrar (RG). Pres only in 1x2 and 3.
