...
# i.e. "Sib_pro_Gud_1_W_NoVeg_1", however, there should only be one. Often this occurs where there is two or more registrators? So then did two people decide to enter the data..? 
...
# Issue solving
dups <- seedling_survival %>% 
  group_by(uniqueID, campaign_number, present) %>%
  tally() %>% 
  filter(n > 1)
