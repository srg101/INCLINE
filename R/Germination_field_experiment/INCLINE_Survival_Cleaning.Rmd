...
full_data <- full_data %>% 
  ...
  fill(veg_height:lichen_cover, .direction = "downup") %>% 
  ungroup()
...
full_data <- full_data %>% 
  select(uniqueID, species, siteID, blockID, experiment, OTC, vegetation, plotID,
         ...
         veg_height, moss_depth, veg_cover, moss_cover, lichen_cover,
         comments)
  
# Save file
write.csv(full_data, "Data/Germination_field_experiment/cleaned/INCLINE_seedling_data.csv")
